...
void handleNewMessage(int fd){
    ...
    if(recvfrom(fd, &mess, sizeof(Message), 0, addr, &aSize) != sizeof(Message))
        print_error("receiving new message failed!");

    if(mess.type == LOGIN){
        tryRegister(fd, addr, aSize, &mess);
    }else if(mess.type == RESULT){
        int resultCtn = ntohl((uint32_t)mess.ctn);
        int result = ntohl((uint32_t)mess.result);
...
void tryRegister(int fd, struct sockaddr *addr, socklen_t aSize, Message* mess){
    ...
    if(cN == 50){
        mType = FAILSIZE;
        if(sendto(fd, &mType, 1, 0, addr, aSize)!= 1) print_error("Failsize message failed!");
        free(addr);
    }else{
        Client* client = findClient(mess->name);
        if(client != NULL){
            mType = FAILNAME;
            if(sendto(fd, &mType, 1, 0, addr, aSize)!= 1) print_error("Failname message failed!");
            free(addr);
        }else{
            ...
            cN++;
            mType = SUCCESS;
            if(sendto(fd, &mType, 1, 0, addr, aSize) != 1) print_error("Success message failed!");
        }
    }
    pthread_mutex_unlock(&clientsLock);
}
...
void* commanderJob(void* arg){
    ...
    while(1){
        ...
        int res = pushCommand(command, op1, op2);
        if(res >= 0){
...
int pushCommand(char command, int op1, int op2){
    ...
    int sfd = getSFD(clients[id].type);
    char mType = REQ;
    ...
    if(sendto(sfd, &mType, 1, 0, clients[id].addr, clients[id].aSize) != 1) ok = -2;
    if(sendto(sfd, &command, 1, 0, clients[id].addr, clients[id].aSize) != 1) ok = -2;
    if(sendto(sfd, &currCtn, sizeof(int), 0, clients[id].addr, clients[id].aSize) != sizeof(int)) ok = -2;
    if(sendto(sfd, &ope1, sizeof(int),0, clients[id].addr, clients[id].aSize) != sizeof(int)) ok = -2;
    if(sendto(sfd, &ope2, sizeof(int),0, clients[id].addr, clients[id].aSize) != sizeof(int)) ok = -2;
